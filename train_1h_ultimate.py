"""
SolarForge Ultimate — 1-Hour Hybrid Model Training Pipeline
============================================================
Combines deep-tree classifiers for C and M classes with a scale-invariant, 
shallow-tree classifier for the rare X-class, targeting a 1-hour horizon.
"""
import pandas as pd
import numpy as np
import warnings
...
def main():
    print("============================================================")
    print("    SolarForge — 1-HOUR ULTIMATE HYBRID ENGINE            ")
    print("============================================================\n")
    ...
    # Shift for 1-hour prediction (360 rows at 10-sec cadence)
    horizon_rows = 360
    ...
    smote_CM = SMOTE(sampling_strategy={1: max(sum(y_train == 1), 30000), 2: 30000}, random_state=42)
    ...
    print(f"\n{'='*65}")
    print(f"  FINAL 1-HOUR ULTIMATE PREDICTION SCORES (TSS)")
    print(f"{'='*65}")
    ...
    joblib.dump(model_data, 'rf_model_1-Hour_Ultimate.pkl')
    print("\n[*] 1-Hour Model Pipeline Saved to: rf_model_1-Hour_Ultimate.pkl")
